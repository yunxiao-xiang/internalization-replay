...
## Q10: markout 符号翻车与更正——真正的风险调整指标

**发生了什么**：临时脚本里把 `-d × Δmid`（d = 公司签名量）标成了 "firm side"，
但 `-d` 恰恰**就是客户签名量**——那张表全是客户视角。基于反号数据我得出了两个
完全错误的结论，都已作废：
- ✗ "宽点差内部化在 2 分钟视角是灾难性亏损"
- ✗ "应给 `min_internalize_spread` 加上界，≥4¢ 全部 route"

**为什么这个错误特别隐蔽**：客户视角负 = 公司赚，是**双重否定**。行业惯例统一用
客户签名（正 = 毒流），我的 `markout.ipynb` 里约定正确（−4.25¢ = benign），
临时脚本却把标签写反。**教训：符号约定必须落进变量名（`client_signed` /
`firm_signed`），不能靠注释**——已按此重写并并入 markout.ipynb（纯 CSV 输入，
任何版本的 fills.csv 都能直接跑，无需重放引擎）。

**更正后的 Markout-adjusted edge（公司视角，¢/股）= 成交边际 + 该视界持仓盈亏**：

| 点差 | 股数 | edge | 5s | 30s | 1min | 5min | 30min |
|---|---|---|---|---|---|---|---|
| 1¢ | 59,000 | +0.50 | +0.45 | +0.72 | +0.02 | −1.48 | **−15.35** |
| 2¢ | 115,800 | 0.00 | −0.15 | −1.58 | −1.05 | −0.46 | +4.79 |
| 3¢ | 28,000 | +0.50 | +1.56 | +2.79 | +3.87 | +5.64 | +23.49 |
| 4¢ | 14,400 | 0.00 | +1.18 | +1.90 | +8.28 | +17.76 | +35.89 |
| 5¢ | 6,100 | +0.50 | −0.68 | +7.66 | +6.50 | +27.78 | +36.24 |
| 6¢ | 5,000 | 0.00 | −3.00 | +14.00 | +42.50 | +146.00 | **+212.50** |
| **全部** | 228,300 | +0.20 | +0.22 | +0.36 | **+1.57** | +5.23 | +10.42 |

美元口径：5s +$507 / 30s +$820 / **1min +$3,589** / 5min +$11,758 / 30min +$21,051。

**四个结论**：
1. **点差越宽风险调整后越赚**（6¢ 桶 +212.5¢ 最优，1¢ 桶唯一转负）——当天宽点差
   是补偿不是警报。这推翻了 write-up 中"wide spreads cut both ways"的**实证**半边；
   **分析**半边（出场也付同一个宽点差）依然成立。
2. **成交边际在风险调整后无足轻重**：30 分钟口径 $400 / $21,051 = **2%**。之前所有
   关于奇偶取整半美分的讨论，都活在这 2% 里面。
3. **1¢ 桶随视界反转**：edge 最高（0.5¢，捕获率 100%）但长视界转负——它是减仓分支，
   平掉的是仍在升值的头寸。**所以 v0.2 的 bleed 削 1¢ 桶：损 edge，护 markout**，
   两个指标按构造就会打架（Q9 的结论需配这个注脚）。
4. **视界是政策选择不是技术细节**：同一批成交 5 秒 +0.22¢、30 分钟 +10.42¢，相差 47 倍。
   视界必须匹配**实际持仓时长**——见下。

### Q10-补充: 实际持仓时间 → 该用哪一档（更正"1 分钟"的判断）

**我先前的推理错了**：用"毛内部化 228,300 股 vs 时间加权敞口 1,559 股 = 周转 146 倍"
推出"持仓很短、1 分钟档才诚实"。**周转率高说明流量大，不等于持仓短**——小而被不断
补充的滚动库存，单股驻留时间仍可以很长。周转衡量的是流量对存量之比，驻留衡量的是
时间，两者不能互推。

**两种独立算法（均只读输出 CSV，已并入 markout.ipynb）**：
- **FIFO 逐股配对**：每股平仓时匹配最早开仓的股 → 股数加权平均 **293 秒**；
- **Little's law**（平均库存 ÷ 单向流量率 = 1,559 ÷ (123,300/23,400)）→ **293 秒**。

两者精确相等不是巧合：稳态下 Little's law 就等价于平均驻留时间，**吻合本身即是
"当天接近稳态、而非一路单边建仓"的证据**。分布（股数加权）：p25 = 82s、
p50 = 219s、p75 = 435s、p90 = 674s，右偏，均值被长尾拉高于中位数。

**正确的档位对应**：

| 视界 | 调整后 edge | 与实际持仓（均值 293s / 中位 219s）的关系 |
|---|---|---|
| 5s / 30s | +0.22 / +0.36¢ | 远短于实际，严重低估 |
| 1min | +1.57¢ | 短于 p25(82s) 之上但远低于均值，仍低估 |
| **5min** | **+5.23¢（$11,758）** | **≈ 均值 293s，主口径** |
| 30min | +10.42¢ | 超过 p90(674s)，高估约 2 倍 |

**报告口径**：主口径 **5 分钟 +5.23¢/股**，保守下限 1 分钟 +1.57¢；中位数 219s 落在
两者之间偏 5 分钟，所以合理区间是 **+3 到 +5¢/股**。双口径报比单点更诚实。

### Q10-补充 2: 1¢ 桶是否全部为减仓？——验证通过

`principal_quote` 在 1¢ 点差下唯一可成交的是 reduce 分支（新增风险被 2¢ 门槛挡住），
上限 `min(order.remaining, abs(position))`、价格为 touch。实证（只读 CSV，重建仓位路径
后逐笔分类）：
...
且 **100% 在 touch 成交**、**零笔大于当时仓位**（不会把空翻多）——`min(..., abs(position))`
这个上限在数据上的直接体现。所以"1¢ 桶 ≡ 减仓业务"是结构性事实，不是当天巧合：
这也解释了它为何 edge 最高（捕获率 100%）却长视界转负（平掉的是仍在升值的头寸）。

**为什么 markout 美元 ≠ drift 美元**（$21,051 vs $14,933，两者都对）：markout 假设
每笔成交的**全量独立持有** h 秒（gross，146 倍周转全部计入）；drift 沿**真实净仓位**
路径积分 pos(t)·dP(t)。两者一致的是符号与量级方向——都在说这天客户流是 uninformed 的，
我们站在压力对面收割了回归，闭合了 Q5 "结构性库存不是巧合"的论证。

**答辩用法**：主动讲这个错误。*"我在 markout 上犯过符号错并得出了完全相反的结论，
是靠与全天 drift 为正的矛盾发现的——两个独立指标互相校验，比任何一个单独可信。
修法不是更小心，是把约定写进变量名。"*
